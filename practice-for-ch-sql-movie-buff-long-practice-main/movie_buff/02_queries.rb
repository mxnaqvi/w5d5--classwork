...
def bad_years
  ...
  # select yr from movies where yr not in (select yr from movies where score > 8 ) group by (yr)
end

def cast_list(title)
  # List all the actors for a particular movie, given the title.
  # Sort the results by starring order (ord). Show the actor id and name.
  Actor.select(:id, :name).joins(:castings).where(movies: {title: title}).order("castings.ord")
end
